...
Como se puede apreciar en los datos obtenidos, el modelo es bastante bueno, ya que el "Accuracy" es de 0.88, lo cual indica que el modelo es bastante bueno, ademas de esto se puede apreciar que el modelo es bastante poderoso y confiable, ya que el modelo es capaz de predecir con un 88% de certeza si una persona tiene sobrepeso o no, ademas de esto se puede apreciar que el modelo es bastante estable, ya que el "Kappa" es de 0.76, lo cual indica que el modelo es bastante estable y confiable.

## Conclusion

El primer modelo realizado el cual consiste en un modelo de regresion lineal multiple para predecir la variable peso, posee una capacidad predictiva exepcionalmente alta, la cual como ya se menciono anteriormente es de aproximadamente 97%, por lo cual su poder predictivo es muy alto, pero no se realizaron verificaciones para evitar sobre ajuste tal como podria ser una validacion cruzada de 5 pliegues o otras variantes, solo se utilizo bootstrap para probar el modelo, aun asi se puede decir que el modelo es confiable debido al gran poder predictivo que posee.

El segundo modelo pese a tener muchos predictores, no tiene un poder predictivo mayor al primero, esto es interesante ya que aunque predicen cosas distintas, el segundo tiene muchos mas predictores que el primero, aun asi tiene un buen poder predictivo, siendo aproximado del 91%, ademas es bastante confiable ya que se realizo una validacion cruzada de 5 pliegues, 5 veces.

El tercer modelo el cual es de regresion logistica, es el que tiene el menor poder preditctivo, siendo solo del 88%, pero es el que es mas confiable, debido a que se realizo una validacion cruzada de 100 pliegues dejando uno fuera en cada caso.
